...
class ResonatorApp:
    def __init__(self, root):
        ...
        ttk.Label(main_frame,
                  text=f"Resonator Optimizer  v{VERSION}",
        ...
        if self.last_results_df is not None:
            try:
                if self.last_filename:
                    default_name = self.last_filename
                else:
    ...
    def _cs_for_max_ratio(self, params_active, row_idx, f_target,
        ...
        if L > 0:
            ...
            cs_min = max(0.01e-12, cs_series_res)   # start at series resonance, search upward
        cs_max = max(cs_max, cs_min * 10)
        ...
        if best_cs < 0.01e-12:
            return 1e-17   # below floor → inactive
    ...
    def _joint_optimise_cs(self, params, n_active, f0,
        ...
        for i in range(n_active):
            ...
            if L > 0:
                cs_series_res = 1.0 / (L * (2.0 * np.pi * f_target) ** 2)
                params[i, 5] = np.clip(cs_series_res * 2.0, 0.01e-12, 1000e-12)
            else:
    ...
    def _run_ga_optimization(self):
        ...
        def _init_plot():
            ...
            mode_label = {"Greedy": "Greedy scan",
                          ...
                          "Greedy+GA": "Greedy + GA refinement"}[mode]
        ...
        def outer_worker():
            """Run the full GA/Greedy search. Rser opt is a separate step (⚙ button)."""
            ga_worker()
    ...
    def _run_rser_optimization(self):
        ...
        def worker():
            ...
            def fom_at(rser_val):
                p = fixed_par.copy()
                self._joint_optimise_cs(p, len(p), f0,
                                        Rload, Cload, Lload, rser_val, R_div2)
                fom = self._compute_fom(
                    p, f0, mags, phases,
                    R_gen, R_div1, R_div2, C_line, Z_in,
        ...
        def _done(best_rser, best_fom, p_final):
            ...
            # Do NOT write Cs back — Cs is Rser-dependent.
            # User must run Cs Optimisation after confirming the new Rser.
            self._cs_optimized = False
            self._led_stop()
            self.status_var.set(
                f"Rser opt done: best Rser={best_rser:.1f} Ω  "
                f"FOM={best_fom:.2f}%  — now run Cs Optimisation")
            try:
                self._optim_rser_btn.configure(state="normal")
                self._optim_run_btn.configure(state="normal")
            except Exception: pass
    ...
    def _build_plot_controls(self, key, parent):
        ...
        if key == 'freq':
            ...
            for m in ("Greedy", "GA only", "Greedy+GA"):
    ...
    def _run_frequency_analysis(self, v_rms_mv, R_gen_int, R_div1, R_div2, C_line, Z_in_meas, Rser, Rload, Cload, Lload, res_params):
        ...
        self.last_filename = f"FreqAnalysis_{timestamp}.csv"
        data = {
    ...
    def _run_steady_state_analysis(self, v_rms_mv, R_gen_int, R_div1, R_div2, C_line, Z_in_meas, Rser, Rload, Cload, Lload, res_params, mags, phases, n_harm):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.last_filename = f"Analysis_FoM={FoM:.2f}_{timestamp}.csv"
