...
CHANGELOG = {
    "1.41": "(1) Steady State: Energy panel replaced by Recycling(t) — cumulative "
            "(Iref²−Iser²)/Iref²_total×100% vs time, converges to Recycling value. "
            "(2) Direct mode checkbox: when ON, R_div1=0 and R_div2=1 GΩ (all "
            "analyses). (3) C_HP (pF, default 1 pF) in parallel with Rser added to "
            "Circuit Parameters; Z_branch = Rser‖C_HP + Z_struct in all calculations; "
            "I_ser is total branch current (includes C_HP current).",
    "1.40": "Added _compute_recycling: frequency-domain metric "
            "Recycling = (Σ|Iref_h|² − Σ|Iser_h|²) / Σ|Iref_h|² × 100 %. "
            "Reference is the same full circuit with Cs=1e-20F (resonators open). "
            "Displayed alongside FOM in the GA/Greedy/SA done status bar.",
...
class ResonatorApp:
    def __init__(self, root):
        ...
        self.C_HP             = tk.DoubleVar(value=1.0)   # pF, in parallel with Rser
        ...
        self._rdiv1_entry = E(params_frame, self.R_div1)
        self._rdiv1_entry.grid(row=2, column=1, padx=2, pady=1)
        L(params_frame, "Rdiv2 Shunt (Ω):").grid(row=2, column=2, sticky="w", padx=PX, pady=1)
        self._rdiv2_entry = E(params_frame, self.R_div2)
        self._rdiv2_entry.grid(row=2, column=3, padx=2, pady=1)
        ...
        # Row 5: C_HP
        L(params_frame, "C_HP ‖ Rser (pF):").grid(row=5, column=0, sticky="w", padx=(0,2), pady=1)
        E(params_frame, self.C_HP).grid(row=5, column=1, padx=2, pady=1)
        ...
        ttk.Checkbutton(fund_frame, text="Direct",
                        variable=self.direct_mode).pack(side=tk.LEFT, padx=(16, 0))
        ...
        self.direct_mode.trace_add("write", self._on_direct_mode_changed)
    ...
    def _populate_resonator_grid(self, params, filter_by_ratio=False):
        ...
        self.resonator_active_vars   = []   # BooleanVar per row; False → Cs=Cp=0
        headers = ("Harm#", "R1", "C1 (F)", "L (H)", "k", "R_DC", "Cs (pF)", "Cp (pF)", "On")
        ...
        for idx, row_data in enumerate(params):
            ...
            # "On" checkbox — unchecked → resonator disabled (Cs=Cp=0 at read time)
            active_var = tk.BooleanVar(value=True)
            ttk.Checkbutton(self.res_params_grid,
                            variable=active_var).grid(row=idx+1, column=8, padx=4)
            self.resonator_active_vars.append(active_var)
    ...
    def _get_resonator_params_from_gui(self):
        ...
        active_vars = getattr(self, 'resonator_active_vars', [])
        rows = []
        ...
        for i, row_widgets in enumerate(self.resonator_entry_widgets):
            ...
            raw[5] *= 1e-12   # Cs: pF -> F
            raw[6] *= 1e-12   # Cp: pF -> F
            # Disabled resonator → zero Cs and Cp so it contributes nothing
            if i < len(active_vars) and not active_vars[i].get():
                raw[5] = 0.0
                raw[6] = 0.0
            if ref_mode:
    ...
    def _cs_for_max_ratio(self, params_active, row_idx, f_target,
        ...
        if L > 0:
            ...
            cs_min = max(0.001e-12, cs_series_res)
        ...
        # at meaninglessly large Cs values — but only if the cap stays above cs_min
        # (otherwise a small Cp would kill the search range and return 1e-17).
        Cp_val = params_active[row_idx, 6]
        if Cp_val > 0:
            cp_cap = 20.0 * Cp_val
            if cp_cap >= cs_min:
                cs_max = min(cs_max, cp_cap)
        if cs_max < cs_min:
        ...
        harm_n = params_active[row_idx, 7]
        if best_ratio <= 1.0 and harm_n > 1:
            return 1e-17   # useless at this harmonic → inactive
        L_val  = params_active[row_idx, 2]
        Cp_val = params_active[row_idx, 6]
        if harm_n > 1 and L_val > 0 and Cp_val > 0 and best_cs > 0:
            f_ar   = np.sqrt((best_cs + Cp_val) / (L_val * best_cs * Cp_val)) / (2 * np.pi)
            f0_est = f_target / harm_n
            if f_ar < f0_est:
    ...
    def _joint_optimise_cs(self, params, n_active, f0,
                           Rload, Cload, Lload, Rser, R_div2,
                           reseed=True, coord_only=False):
        """
        Jointly re-optimise Cs[0..n_active-1] using coordinate descent.

        Rows are processed in Harm# order (read directly from params[:,7]).
        Row ordering in params is NEVER changed — harm_order is used only as
        an index sequence.

        reseed=True    : Step 1 seeds Cs from series-resonance values.
        reseed=False   : skip Step 1.
        coord_only=True: skip Steps 1+2, run only coordinate descent (Step 3).

        Step 1 — seed Cs for each slot.
        ...
        # Indices into params[:n_active] sorted by Harm# — never reorder rows.
        harm_order = np.argsort(params[:n_active, 7], kind='stable')
        ...
        if not coord_only:
            ...
            for step, ii in enumerate(harm_order):
                subset = params[harm_order[:step + 1]]   # copy in Harm# order
                params[ii, 5] = self._cs_for_max_ratio(
                    subset, step, params[ii, 7] * f0,
                    Rload, Cload, Lload, Rser, R_div2)
        ...
        for _ in range(MAX_ROUNDS):
            ...
            for step, ii in enumerate(harm_order):
                all_sorted = params[harm_order]          # fresh copy each step
                params[ii, 5] = self._cs_for_max_ratio(
                    all_sorted, step, params[ii, 7] * f0,
                    Rload, Cload, Lload, Rser, R_div2)
    ...
    def _update_grid_cs(self, params):
        ...
        Matches by harm# so row order differences between GUI and params don't
        cause Cs values to be written to the wrong inductor."""
        harm_nums = getattr(self, 'resonator_harm_nums', [])
        cs_by_harm = {int(round(params[i, 7])): params[i, 5]
                      for i in range(len(params))}
        for i, row_w in enumerate(self.resonator_entry_widgets):
            h = harm_nums[i] if i < len(harm_nums) else i + 1
            if h not in cs_by_harm:
                continue
            row_w[5].delete(0, tk.END)
            row_w[5].insert(0, f"{cs_by_harm[h] * 1e12:.6g}")

    def _rdiv(self):
        """Return (R_div1, R_div2) — overridden to (0, 1e9) when Direct mode is ON."""
        if self.direct_mode.get():
            return 0.0, 1e9
        return self.R_div1.get(), self.R_div2.get()

    def _on_direct_mode_changed(self, *_):
        """Dim/enable Rdiv entry fields based on Direct mode state."""
        state = "disabled" if self.direct_mode.get() else "normal"
        try:
            self._rdiv1_entry.configure(state=state)
            self._rdiv2_entry.configure(state=state)
        except Exception:
            pass
    ...
    def _run_cs_optimization(self):
        ...
        def worker():
            ...
            # ── Step 1: zero all Cs (resonators effectively open) ────────────
            p[:, 5] = 1e-20

            # ── Step 2: forward pass — one harmonic at a time ────────────────
            # All N resonators are always in the circuit; untuned ones have
            # Cs≈0 → series cap open → no contribution at that harmonic.
            for step, ii in enumerate(harm_order):
                f_target_k = p[ii, 7] * f0
                q.put(('status',
                       f"Pass {step+1}/{N}: Harm#{int(p[ii,7])} — "
                       f"scanning Cs at {f_target_k/1e6:.2f} MHz…"))
                # Find Cs for resonator ii while all others are fixed
                p[ii, 5] = self._cs_for_max_ratio(
                    p, ii, f_target_k,
                    Rload, Cload, Lload, Rser, R_div2)

                snap = p[harm_order].copy()
                ratio_vals = np.array([
                ...
                Z_curves = [
                    np.array([abs(self.calculate_single_resonator_impedance(f, snap[j]))
                               for f in f_plot])
                    for j in range(N)]
                step_label = f"Harm#{int(p[ii,7])}: Cs={p[ii,5]*1e12:.3g}pF"
                q.put(('step', step, snap, ratio_vals, Z_curves, step_label))

            # ── Step 3: subsequent rounds — tweak each Cs, plot every step ──
            MAX_ROUNDS = 8 if not self._fast_eval else 4
            for rnd in range(1, MAX_ROUNDS + 1):
                prev_cs = p[harm_order, 5].copy()
                for step, ii in enumerate(harm_order):
                    f_target_k = p[ii, 7] * f0
                    q.put(('status',
                           f"Round {rnd+1}/{MAX_ROUNDS+1}: "
                           f"Harm#{int(p[ii,7])} — tweaking Cs…"))
                    p[ii, 5] = self._cs_for_max_ratio(
                        p, ii, f_target_k,
                        Rload, Cload, Lload, Rser, R_div2)
                    ...
                    Z_curves = [
                        np.array([abs(self.calculate_single_resonator_impedance(f, snap[j]))
                                   for f in f_plot])
                        for j in range(N)]
                    step_label = (f"R{rnd+1} Harm#{int(p[ii,7])}: "
                                  f"Cs={p[ii,5]*1e12:.3g}pF")
                    q.put(('refine', step, snap, ratio_vals, Z_curves, step_label))

                # Convergence check after each full round
                new_cs = p[harm_order, 5]
                if np.all(np.abs(new_cs - prev_cs) /
                          np.maximum(np.abs(prev_cs), 1e-20) < 0.005):
                    break
    ...
    def _compute_dvdt_waveforms(self, res_params, f0, mags_in, phases,
                                R_gen_int, R_div1, R_div2, C_line,
                                Z_in_meas, Rser, Rload, Cload, Lload):
        """
        Compute time-domain waveforms over one fundamental period:
          Vout_t     — load voltage with resonators
          Vout_ref_t — load voltage without resonators
          Vin_t      — source stimulus voltage
          dVout_dt, dVout_ref_dt, dVin_dt — their first derivatives
        Returns (t, Vout_t, Vout_ref_t, Vin_t, dVout_dt, dVout_ref_dt, dVin_dt).
        """
        w0 = 2 * np.pi * f0
        n_harm = len(phases)
        ...
        Vout_t       = np.zeros(512)
        Vout_ref_t   = np.zeros(512)
        Vin_t        = np.zeros(512)
        dVout_dt     = np.zeros(512)
        dVout_ref_dt = np.zeros(512)
        dVin_dt      = np.zeros(512)
        ...
        for n in range(1, n_harm + 1):
            ...
            Y_res = sum(1.0/self.calculate_single_resonator_impedance(f_h, p)
                        for p in res_params if p[5] >= 1e-16)
            ...
            Z_node   = 1.0/Y_node
            V_node   = V_src * Z_node / (R_gen_int + R_div1 + Z_node)
            Iser_ph  = V_node / Z_branch
            Vload_ph = Iser_ph * Z_struct

            Z_branch_r   = Rser + Z_load_h
            Y_node_r     = Y_shunt + 1.0/Z_branch_r
            Z_node_r     = 1.0/Y_node_r
            V_node_r     = V_src * Z_node_r / (R_gen_int + R_div1 + Z_node_r)
            Iref_ph      = V_node_r / Z_branch_r
            Vload_ref_ph = Iref_ph * Z_load_h

            amp_out = abs(Vload_ph);      phi_out = np.angle(Vload_ph)
            amp_ref = abs(Vload_ref_ph);  phi_ref = np.angle(Vload_ref_ph)
            amp_in  = abs(V_src);         phi_in  = np.angle(V_src)

            Vout_t       += amp_out * np.cos(w_h*t + phi_out)
            Vout_ref_t   += amp_ref * np.cos(w_h*t + phi_ref)
            Vin_t        += amp_in  * np.cos(w_h*t + phi_in)
            dVout_dt     += -w_h * amp_out * np.sin(w_h*t + phi_out)
            dVout_ref_dt += -w_h * amp_ref * np.sin(w_h*t + phi_ref)
            dVin_dt      += -w_h * amp_in  * np.sin(w_h*t + phi_in)

        return t, Vout_t, Vout_ref_t, Vin_t, dVout_dt, dVout_ref_dt, dVin_dt
    ...
    def _compute_fom(self, res_params, f0, mags_in, phases,
        ...
        C_HP_val = self.C_HP.get() * 1e-12   # capacitor in parallel with Rser
        ...
        for n in range(1, n_harm + 1):
            ...
            Z_HP     = Rser / (1.0 + 1j * w_h * Rser * C_HP_val)  # Rser ‖ C_HP
            Y_div2   = 1.0/R_div2 + 1j*w_h*C_line
            Y_shunt  = Y_div2 + 1.0/Z_in_meas
            Z_load_h = Rload + 1j*w_h*Lload + 1.0/(1j*w_h*Cload)
    ...
    def _compute_recycling(self, res_params, f0, mags_in, phases,
                           R_gen_int, R_div1, R_div2, C_line,
                           Z_in_meas, Rser, Rload, Cload, Lload):
        """
        Recycling (%) = (Σ|Iref_h|² − Σ|Iser_h|²) / Σ|Iref_h|² × 100

        Iref_h : phasor current in Rser branch at harmonic h with all Cs = 1e-20 F
                 (resonators open-circuited — same circuit topology, no resonators).
        Iser_h : phasor current with active resonators.

        Computed in the frequency domain (one phasor per harmonic) — no time-domain
        simulation needed.  Same stimulus scaling as _compute_fom.
        """
        w0     = 2 * np.pi * f0
        n_harm = len(phases)

        # Same magnitude scaling as _compute_fom
        t_sc  = np.linspace(0, 1/f0, 512, endpoint=False)
        V_raw = np.full(512, mags_in[0], dtype=float)
        for i in range(n_harm):
            V_raw += mags_in[i+1] * np.cos((i+1)*w0*t_sc + np.deg2rad(phases[i]))
        ...
        for n in range(1, n_harm + 1):
            ...
            Z_HP     = Rser / (1.0 + 1j * w_h * Rser * C_HP_val)
            Y_div2   = 1.0/R_div2 + 1j*w_h*C_line
            Y_shunt  = Y_div2 + 1.0/Z_in_meas
            Z_load_h = Rload + 1j*w_h*Lload + 1.0/(1j*w_h*Cload)
            ...
            Y_node   = Y_shunt + 1.0/Z_branch
            Z_node   = 1.0/Y_node
            V_node   = V_src * Z_node / (R_gen_int + Z_node + R_div1)
            Iser_ph  = V_node / Z_branch

            # Reference: Cs = 1e-20 F → resonators are open → Z_res = ∞
            Z_branch_r = Z_HP + Z_load_h
            Y_node_r   = Y_shunt + 1.0/Z_branch_r
            Z_node_r   = 1.0/Y_node_r
            V_node_r   = V_src * Z_node_r / (R_gen_int + Z_node_r + R_div1)
            Iref_ph    = V_node_r / Z_branch_r

            I2_ser += abs(Iser_ph)**2
            I2_ref += abs(Iref_ph)**2

        return (I2_ref - I2_ser) / I2_ref * 100.0 if I2_ref > 1e-30 else 0.0
    ...
    def _save_ga_progress_prompt(self, gen_best, gen_mean, names, best_fom, recycling=None):
        ...
        rec_str = f"  Recycling={recycling:.2f}%" if recycling is not None else ""
        self.status_var.set(
            f"Search done. FOM={best_fom:.2f}%{rec_str}  "
    ...
    def _run_ga_optimization(self):
        ...
        def _init_dvdt_plots():
            fig1, (ax_v,), cv1 = self._get_plot_window(
                'ga_vout', 'GA — Load Voltage', (8, 4), 1)
            ax_v.set_xlabel("Time (µs)")
            ax_v.set_ylabel("Voltage (a.u.)")
            ax_v.set_title("Load Voltage: with resonators / reference / stimulus")
            ax_v.grid(True, alpha=0.4)
            lv,  = ax_v.plot([], [], 'b-',  lw=1.5, label="Vout (with res)")
            lvr, = ax_v.plot([], [], 'r--', lw=1.0, label="Vout (no res)")
            lvi, = ax_v.plot([], [], 'g:',  lw=1.0, label="Vin (stimulus)")
            ax_v.legend(fontsize=8)
            fig1.tight_layout(); cv1.draw()

            fig2, (ax_d,), cv2 = self._get_plot_window(
                'ga_dvdt', 'GA — dV/dt', (8, 4), 1)
            ax_d.set_xlabel("Time (µs)")
            ax_d.set_ylabel("dV/dt (a.u./s)")
            ax_d.set_title("|dVout/dt| with resonators vs reference vs stimulus slope")
            ax_d.grid(True, alpha=0.4)
            ld,  = ax_d.plot([], [], 'b-',  lw=1.5, label="|dVout/dt| with res")
            ldr, = ax_d.plot([], [], 'r--', lw=1.0, label="|dVout/dt| no res")
            ldi, = ax_d.plot([], [], 'g:',  lw=1.0, label="|dVin/dt| stimulus")
            ax_d.legend(fontsize=8)
            fig2.tight_layout(); cv2.draw()

            _ax_vout[0]    = ax_v;    _ax_dvdt[0]    = ax_d
            _line_vout[0]  = lv;      _line_vref[0]  = lvr;  _line_vin[0]  = lvi
            _line_dvout[0] = ld;      _line_dvref[0] = ldr;  _line_dvin[0] = ldi
            _dvdt_canvases[0] = (cv1, cv2)
            _dvdt_ready[0] = True

        def _update_dvdt_plots(best_par):
            if not _dvdt_ready[0]:
                _init_dvdt_plots()
            if best_par is None or len(best_par) == 0:
                return
            try:
                mags_d, phases_d, _ = self._load_stimulus()
            except Exception:
                return
            active = best_par[best_par[:, 5] >= 1e-16]
            if len(active) == 0:
                return
            t, Vo, Vr, Vi, dVo, dVr, dVi = self._compute_dvdt_waveforms(
                active, f0, mags_d, phases_d,
                self.R_internal_gen.get(), self._rdiv()[0],
                R_div2, 0.0, self.Z_in_measure.get(),
                _rser_cell[0], Rload, Cload, Lload)
            t_us = t * 1e6
            _line_vout[0].set_data(t_us, Vo)
            _line_vref[0].set_data(t_us, Vr)
            _line_vin[0].set_data(t_us, Vi)
            _ax_vout[0].set_xlim(t_us[0], t_us[-1])
            vall = np.concatenate([Vo, Vr, Vi])
            margin = max(abs(vall).max() * 0.15, 1e-12)
            _ax_vout[0].set_ylim(vall.min() - margin, vall.max() + margin)

            _line_dvout[0].set_data(t_us, np.abs(dVo))
            _line_dvref[0].set_data(t_us, np.abs(dVr))
            _line_dvin[0].set_data(t_us, np.abs(dVi))
            _ax_dvdt[0].set_xlim(t_us[0], t_us[-1])
            dall = np.concatenate([np.abs(dVo), np.abs(dVr), np.abs(dVi)])
            _ax_dvdt[0].set_ylim(0, dall.max() * 1.15)

            for cv in _dvdt_canvases[0]:
                cv.draw()
        ...
        def poll_ga():
            ...
            while True:
                ...
                if msg[0] == 'status':
                    ...
                    recycling = 0.0
                    if best_par is not None and len(best_par) > 0:
                        recycling = self._compute_recycling(
                            best_par, f0, mags, phases,
                            R_gen, R_div1, R_div2, C_line, Z_in,
                            _rser_cell[0], Rload, Cload, Lload)
                    self._ga_running = False
                    self._fast_eval = False
                    ...
                    self._save_ga_progress_prompt(gb_final, gm_final, names, best_fom, recycling)
                    done_flag = True
                    break
    ...
    def _led_done(self):
        """Stop flashing and show bright green — Best Inductors finished."""
        self._led_on = False
        if self._led_job:
            self.root.after_cancel(self._led_job)
            self._led_job = None
        try:
            self._led_canvas.itemconfig(self._led, fill="#00ff55")
        except Exception:
            pass
    ...
    def run_analysis(self):
        ...
        try:
            ...
            R_div1, R_div2 = self._rdiv(); C_line = 0.0  # C_line fixed = 0
    ...
    def _run_frequency_analysis(self, v_rms_mv, R_gen_int, R_div1, R_div2, C_line, Z_in_meas, Rser, Rload, Cload, Lload, res_params):
        ...
        for f in freqs:
            ...
            C_HP_val = self.C_HP.get() * 1e-12
            Z_HP     = Rser / (1.0 + 1j * w * Rser * C_HP_val) if w > 0 else Rser
            Z_branch = Z_HP + Z_struct
            ...
            Z_branch_ref = Z_HP + Z_struct_ref
    ...
    def _run_steady_state_analysis(self, v_rms_mv, R_gen_int, R_div1, R_div2, C_line, Z_in_meas, Rser, Rload, Cload, Lload, res_params, mags, phases, n_harm):
        ...
        I2_ser_ph = 0.0   # Σ|Iser_h|² — for Recycling
        I2_ref_ph = 0.0   # Σ|Iref_h|²
        ...
        for n in range(n_harm + 1):
            ...
            I2_ser_ph += abs(Iser_phasor_h)**2
            I2_ref_ph += abs(Iref_phasor_h)**2
        ...
        Recycling = (I2_ref_ph - I2_ser_ph) / I2_ref_ph * 100 if I2_ref_ph > 1e-30 else 0.0
        ...
        self.axes[2].plot(t*1e9, Iref_t*1000, color='gray', linestyle=':', label='I_ref (no res)')
        self.axes[2].plot(t*1e9, Iser_t*1000, color='black', linewidth=1.5, label='I_ser')
        self.axes[2].text(0.02, 0.95, f'Recycling = {Recycling:.2f}%',
                          transform=self.axes[2].transAxes,
                          fontsize=11, fontweight='bold', color='purple',
                          verticalalignment='top')
        self.axes[2].set_title('Recycling'); self.axes[2].set_ylabel('mA'); self.axes[2].legend(fontsize=7, loc='upper right'); self.axes[2].grid(True)
