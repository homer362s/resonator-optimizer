...
class ResonatorApp:
    ...
    def _run_rser_optimization(self):
        ...
        def worker():
            ...
            def fom_at(rser_val):
                # Full Cs re-optimisation at each candidate Rser (reseed=True)
                # so FOM is computed with the best possible Cs for that Rser —
                # same as the Best Inductors search. Without this the FOM at
                # any Rser != original is computed with suboptimal Cs and the
                # search cannot find a true improvement.
                p = fixed_par.copy()
