...
class ResonatorApp:
    ...
    def _build_plot_controls(self, key, parent):
        ...
        if key == 'freq':
            ...
            self._prop_f0_lbl = ttk.Label(_prop_inner, text="f₀≈ —", foreground="blue")
            ...
            def _on_prop_inductor_selected(event=None):
                ...
                try:
                    ...
                    L_val  = params[0, 2]
                    Cp_val = params[0, 6]
                    if Cs_opt > 0 and L_val > 0 and Cp_val > 0:
                        f_ar = (np.sqrt((Cs_opt + Cp_val) /
                                        (L_val * Cs_opt * Cp_val)) / (2 * np.pi))
                        f0_mhz = round(f_ar / 1e6, 3)
                        self._prop_f0_lbl.configure(text=f"f₀≈ {f0_mhz} MHz")
                        self.f_fundamental.set(f0_mhz)
                    else:
                        self._prop_f0_lbl.configure(text="f₀≈ —")
                except Exception:
                    self._prop_f0_lbl.configure(text="f₀≈ err")
